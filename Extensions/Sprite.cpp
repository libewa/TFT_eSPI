...
***************************************************************************************/
// cast returned value to (unsigned char*) for 8-bit or (unsigned short*) for 16-bit colours
void* TFT_eSprite::createSprite(short w, short h, unsigned char frames)
{
  ...
  _img8   = (unsigned char*) callocSprite(w, h, frames);
  _img8_1 = _img8;
  _img8_2 = _img8;
  _img    = (unsigned short*) _img8;
  _img4   = _img8;
...
***************************************************************************************/
void* TFT_eSprite::callocSprite(short w, short h, unsigned char frames)
{
  ...
  if (_bpp == 16)
  {
    ...
    {
      ptr8 = ( unsigned char*) ps_calloc(frames * w * h + frames, sizeof(unsigned short));
      //Serial.println("PSRAM");
    }
    else
#endif
    {
      ptr8 = ( unsigned char*) calloc(frames * w * h + frames, sizeof(unsigned short));
  ...
  {
#if defined (ESP32) && defined (CONFIG_SPIRAM_SUPPORT)
    if ( psramFound() && _psram_enable ) ptr8 = ( unsigned char*) ps_calloc(frames * w * h + frames, sizeof(unsigned char));
    else
#endif
    ptr8 = ( unsigned char*) calloc(frames * w * h + frames, sizeof(unsigned char));
  }
  ...
  else if (_bpp == 4)
  {
    ...
    _iwidth = w;
#if defined (ESP32) && defined (CONFIG_SPIRAM_SUPPORT)
    if ( psramFound() && _psram_enable ) ptr8 = ( unsigned char*) ps_calloc(((frames * w * h) >> 1) + frames, sizeof(unsigned char));
    else
#endif
    ptr8 = ( unsigned char*) calloc(((frames * w * h) >> 1) + frames, sizeof(unsigned char));
  }
...
#if defined (ESP32) && defined (CONFIG_SPIRAM_SUPPORT)
    if ( psramFound() && _psram_enable ) ptr8 = ( unsigned char*) ps_calloc(frames * (w>>3) * h + frames, sizeof(unsigned char));
    else
#endif
    ptr8 = ( unsigned char*) calloc(frames * (w>>3) * h + frames, sizeof(unsigned char));
  }
...
***************************************************************************************/
void TFT_eSprite::createPalette(unsigned short colorMap[], unsigned char colors)
{
  if (!_created) return;
  ...
  if (_colorMap == nullptr) _colorMap = (unsigned short *)calloc(16, sizeof(unsigned short));
...
** Description:             Set a palette for a 4-bit per pixel sprite
***************************************************************************************/
void TFT_eSprite::createPalette(const unsigned short colorMap[], unsigned char colors)
{
  if (!_created) return;
...
void* TFT_eSprite::frameBuffer(char f)
{
  if (!_created) return nullptr;
  ...
  if (_bpp == 16) _img = (unsigned short*)_img8;
...
***************************************************************************************/
void* TFT_eSprite::setColorDepth(char b)
{
...
***************************************************************************************/
char TFT_eSprite::getColorDepth(void)
{
...
***************************************************************************************/
void TFT_eSprite::setBitmapColor(unsigned short c, unsigned short b)
{
  if (c == b) b = ~c;
  _tft->bitmap_fg = c;
  _tft->bitmap_bg = b;
...
***************************************************************************************/
void TFT_eSprite::setPaletteColor(unsigned char index, unsigned short color)
{
...
***************************************************************************************/
unsigned short TFT_eSprite::getPaletteColor(unsigned char index)
{
...
***************************************************************************************/
#define FP_SCALE 10
bool TFT_eSprite::pushRotated(short angle, unsigned int transp)
{
  ...
  unsigned short sline_buffer[max_x - min_x + 1];

  int xt = min_x - _tft->_xPivot;
  int yt = min_y - _tft->_yPivot;
  unsigned int xe = _dwidth << FP_SCALE;
  unsigned int ye = _dheight << FP_SCALE;
  unsigned short tpcolor = (unsigned short)transp;
  ...
  for (int y = min_y; y <= max_y; y++, yt++) {
    int x = min_x;
    unsigned int xs = (_cosra * xt - (_sinra * yt - (_xPivot << FP_SCALE)) + (1 << (FP_SCALE - 1)));
    unsigned int ys = (_sinra * xt + (_cosra * yt + (_yPivot << FP_SCALE)) + (1 << (FP_SCALE - 1)));
    ...
    do {
      ...
      else { rp = readPixel(xp, yp); rp = (unsigned short)(rp>>8 | rp<<8); }
...
bool TFT_eSprite::pushRotated(TFT_eSprite *spr, short angle, unsigned int transp)
{
  ...
  int xt = min_x - spr->_xPivot;
  int yt = min_y - spr->_yPivot;
  unsigned int xe = _dwidth << FP_SCALE;
  unsigned int ye = _dheight << FP_SCALE;
  unsigned short tpcolor = (unsigned short)transp;
...
***************************************************************************************/
bool TFT_eSprite::getRotatedBounds(short angle, short *min_x, short *min_y,
...
***************************************************************************************/
bool TFT_eSprite::getRotatedBounds(TFT_eSprite *spr, short angle, short *min_x, short *min_y,
                                                                    short *max_x, short *max_y)
{
  // Get the bounding box of this rotated source Sprite relative to Sprite pivot
  getRotatedBounds(angle, width(), height(), _xPivot, _yPivot, min_x, min_y, max_x, max_y);
...
***************************************************************************************/
void TFT_eSprite::getRotatedBounds(short angle, short w, short h, short xp, short yp,
                                   short *min_x, short *min_y, short *max_x, short *max_y)
{
  ...
  short x0 = -xp * cosa - yp * sina;
  short y0 =  xp * sina - yp * cosa;

  short x1 =  w * cosa - yp * sina;
  short y1 = -w * sina - yp * cosa;

  short x2 =  h * sina + w * cosa;
  short y2 =  h * cosa - w * sina;

  short x3 =  h * sina - xp * cosa;
  short y3 =  h * cosa + xp * sina;
...
***************************************************************************************/
void TFT_eSprite::pushSprite(int x, int y, unsigned short transp)
{
  if (!_created) return;
  ...
  {
    transp = (unsigned char)((transp & 0xE000)>>8 | (transp & 0x0700)>>6 | (transp & 0x0018)>>3);
    _tft->pushImage(x, y, _dwidth, _dheight, _img8, (unsigned char)transp, (bool)true);
  }
  else if (_bpp == 4)
  {
    _tft->pushImage(x, y, _dwidth, _dheight, _img4, (unsigned char)(transp & 0x0F), false, _colorMap);
  }
...
bool TFT_eSprite::pushToSprite(TFT_eSprite *dspr, int x, int y)
{
  if (!_created) return false;
  if (!dspr->created()) return false;

  // Check destination sprite compatibility
  char ds_bpp = dspr->getColorDepth();
...
bool TFT_eSprite::pushToSprite(TFT_eSprite *dspr, int x, int y, unsigned short transp)
{
  ...
  bool oldSwapBytes = dspr->getSwapBytes();
  unsigned short sline_buffer[width()];
...
***************************************************************************************/
bool TFT_eSprite::pushSprite(int tx, int ty, int sx, int sy, int sw, int sh)
{
  if (!_created) return false;
  ...
  */
  ...
  else if (_bpp == 4)
  {
    ...
    {
      int ds = _xs&1; // Odd x start pixel

      int de = 0;     // Odd x end pixel
      if ((sw > ds) && (_xe&1)) de = 1;

      unsigned int dm = 0;     // Midsection pixel count
      if (sw > (ds+de)) dm = sw - ds - de;
      sw--;

      unsigned int yp = (_xs + ds + _iwidth * _ys)>>1;
      _tft->startWrite();
      while (sh--)
      {
...
***************************************************************************************/
unsigned short TFT_eSprite::readPixelValue(int x, int y)
{
...
***************************************************************************************/
unsigned short TFT_eSprite::readPixel(int x, int y)
{
  ...
  if (_bpp == 16)
  {
    unsigned short color = _img[x + y * _iwidth];
    return (color >> 8) | (color << 8);
  }

  if (_bpp == 8)
  {
    unsigned short color = _img8[x + y * _iwidth];
    if (color != 0)
    {
    unsigned char  blue[] = {0, 11, 21, 31};
  ...
  unsigned short color = (_img8[(x + y * _bitwidth)>>3] << (x & 0x7)) & 0x80;
...
***************************************************************************************/
void  TFT_eSprite::pushImage(int x, int y, int w, int h, unsigned short *data, unsigned char sbpp)
{
  ...
  if (_bpp == 16) // Plot a 16 bpp image into a 16 bpp Sprite
  {
    ...
    unsigned char *ptro = (unsigned char *)data + ((dx + dy * w) << 1);
    // Pointer within sprite image
    unsigned char *ptrs = (unsigned char *)_img + ((x + y * _iwidth) << 1);
  ...
  {
    // Pointer within original image
    unsigned char *ptro = (unsigned char *)data + (dx + dy * w);
    // Pointer within sprite image
    unsigned char *ptrs = (unsigned char *)_img + (x + y * _iwidth);
  ...
  {
    unsigned short lastColor = 0;
    unsigned char  color8    = 0;
    for (int yp = dy; yp < dy + dh; yp++)
    {
      ...
      {
        unsigned short color = data[dxypw++];
        if (color != lastColor) {
          // When data source is a sprite, the bytes are already swapped
          if(!_swapBytes) color8 = (unsigned char)((color & 0xE0) | (color & 0x07)<<2 | (color & 0x1800)>>11);
          else color8 = (unsigned char)((color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3);
        }
        lastColor = color;
        _img8[xyw++] = color8;
      }
  ...
  else // 1bpp
  {
    ...
    for (int yp = dy;  yp < dy + dh; yp++)
    {
      unsigned int yw = yp * ww;              // Byte starting the line containing source pixel
      int ox = x;
      for (int xp = dx; xp < dx + dw; xp++)
      {
        unsigned short readPixel = (ptr[(xp>>3) + yw] & (0x80 >> (xp & 0x7)) );
...
***************************************************************************************/
void  TFT_eSprite::pushImage(int x, int y, int w, int h, const unsigned short *data)
{
#ifdef ESP32
  pushImage(x, y, w, h, (unsigned short*) data);
#else
  ...
  if (_bpp == 16) // Plot a 16 bpp image into a 16 bpp Sprite
  {
    for (int yp = dy; yp < dy + dh; yp++)
    {
      ...
      for (int xp = dx; xp < dx + dw; xp++)
      {
        unsigned short color = pgm_read_word(data + xp + yp * w);
  ...
  else if (_bpp == 8) // Plot a 16 bpp image into a 8 bpp Sprite
  {
    for (int yp = dy; yp < dy + dh; yp++)
    {
      ...
      for (int xp = dx; xp < dx + dw; xp++)
      {
        unsigned short color = pgm_read_word(data + xp + yp * w);
        if(_swapBytes) color = color<<8 | color>>8;
        _img8[ox + y * _iwidth] = (unsigned char)((color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3);
        ox++;
      }
      y++;
    }
  ...
  else if (_bpp == 4)
  {
    ...
    Serial.println("TFT_eSprite::pushImage(int x, int y, int w, int h, const unsigned short *data) not implemented");
    #endif
    return;
  }
  ...
  else // Plot a 1bpp image into a 1bpp Sprite
  {
    ...
    y-= _yDatum;
    unsigned short bsw =  (w+7) >> 3; // Width in bytes of source image line
    unsigned char *ptr = ((unsigned char*)data) + dy * bsw;
    
    while (dh--) {
      int odx = dx;
      int ox  = x;
      while (odx < dx + dw) {
        unsigned char pbyte = pgm_read_byte(ptr + (odx>>3));
        unsigned char mask = 0x80 >> (odx & 7);
        while (mask) {
          unsigned char p = pbyte & mask;
          mask = mask >> 1;
          drawPixel(ox++, y, p);
          odx++;
        }
...
void TFT_eSprite::setWindow(int x0, int y0, int x1, int y1)
{
...
***************************************************************************************/
void TFT_eSprite::pushColor(unsigned short color)
{
  if (!_created ) return;

  // Write the colour to RAM in set window
  if (_bpp == 16)
    _img [_xptr + _yptr * _iwidth] = (unsigned short) (color >> 8) | (color << 8);

  else  if (_bpp == 8)
    _img8[_xptr + _yptr * _iwidth] = (unsigned char )((color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3);
...
***************************************************************************************/
void TFT_eSprite::pushColor(unsigned short color, unsigned int len)
{
  if (!_created ) return;
  ...
  if (_bpp == 16)
    pixelColor = (unsigned short) (color >> 8) | (color << 8);

  else  if (_bpp == 8)
    pixelColor = (color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3;

  else pixelColor = (unsigned short) color; // for 1bpp or 4bpp
...
***************************************************************************************/
void TFT_eSprite::writeColor(unsigned short color)
{
  if (!_created ) return;
  ...
  else if (_bpp == 8) _img8[_xptr + _yptr * _iwidth] = (unsigned char) color;
...
void TFT_eSprite::setScrollRect(int x, int y, int w, int h, unsigned short color)
{
...
***************************************************************************************/
void TFT_eSprite::scroll(short dx, short dy)
{
  ...
  unsigned int w  = _sw - abs(dx); // line width to copy
  unsigned int h  = _sh - abs(dy); // lines to copy
  int iw  = _iwidth;       // rounded up width of sprite

  // Fetch the x,y origin set by setScrollRect()
  unsigned int tx = _sx; // to x
  unsigned int fx = _sx; // from x
  unsigned int ty = _sy; // to y
  unsigned int fy = _sy; // from y
  ...
  unsigned int fyp = fx + fy * _iwidth;
  unsigned int typ = tx + ty * _iwidth;
  ...
  else if (_bpp == 4)
  {
    ...
    { // move pixels one by one
      for (unsigned short xp = 0; xp < w; xp++)
      {
...
***************************************************************************************/
void TFT_eSprite::fillSprite(unsigned int color)
{
  ...
  {
    if(_bpp == 16) {
      if ( (unsigned char)color == (unsigned char)(color>>8) ) {
        memset(_img,  (unsigned char)color, _iwidth * _yHeight * 2);
      }
    ...
    else if (_bpp == 8)
    {
      ...
      memset(_img8, (unsigned char)color, _iwidth * _yHeight);
    }
    else if (_bpp == 4)
    {
      unsigned char c = ((color & 0x0F) | (((color & 0x0F) << 4) & 0xF0));
...
void TFT_eSprite::setRotation(unsigned char r)
{
  if (_bpp != 1) return;
...
***************************************************************************************/
unsigned char TFT_eSprite::getRotation(void)
{
  return rotation;
}
...
***************************************************************************************/
void TFT_eSprite::drawPixel(int x, int y, unsigned int color)
{
  if (!_created || _vpOoB) return;
  ...
  if (_bpp == 16)
  {
    ...
    _img[x+y*_iwidth] = (unsigned short) color;
  }
  else if (_bpp == 8)
  {
    _img8[x+y*_iwidth] = (unsigned char)((color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3);
  }
  else if (_bpp == 4)
  {
    unsigned char c = color & 0x0F;
    int index = (x+y*_iwidth)>>1;;
    if ((x & 0x01) == 0) {
      _img4[index] = (unsigned char)((c << 4) | (_img4[index] & 0x0F));
    }
    else {
      _img4[index] =  (unsigned char)(c | (_img4[index] & 0xF0));
    }
  }
  else // 1 bpp
  {
...
***************************************************************************************/
void TFT_eSprite::drawLine(int x0, int y0, int x1, int y1, unsigned int color)
{
  if (!_created || _vpOoB) return;
...
***************************************************************************************/
void TFT_eSprite::drawFastVLine(int x, int y, int h, unsigned int color)
{
  if (!_created || _vpOoB) return;
  ...
  if (_bpp == 16)
  {
    color = (color >> 8) | (color << 8);
    int yp = x + _iwidth * y;
    while (h--) {_img[yp] = (unsigned short) color; yp += _iwidth;}
  }
  else if (_bpp == 8)
  {
    color = (color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3;
    while (h--) _img8[x + _iwidth * y++] = (unsigned char) color;
  }
  else if (_bpp == 4)
  {
    if ((x & 0x01) == 0)
    {
      unsigned char c = (unsigned char) (color & 0xF) << 4;
      while (h--) {
        _img4[(x + _iwidth * y)>>1] = (unsigned char) (c | (_img4[(x + _iwidth * y)>>1] & 0x0F));
        y++;
      }
    }
    else {
      unsigned char c = (unsigned char)color & 0xF;
      while (h--) {
        _img4[(x + _iwidth * y)>>1] = (unsigned char) (c | (_img4[(x + _iwidth * y)>>1] & 0xF0)); // x is odd; new color goes into the low bits.
        y++;
      }
    }
  }
...
***************************************************************************************/
void TFT_eSprite::drawFastHLine(int x, int y, int w, unsigned int color)
{
  if (!_created || _vpOoB) return;
  ...
  {
    color = (color >> 8) | (color << 8);
    while (w--) _img[_iwidth * y + x++] = (unsigned short) color;
  }
  else if (_bpp == 8)
  {
    color = (color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3;
    memset(_img8+_iwidth * y + x, (unsigned char)color, w);
  }
  else if (_bpp == 4)
  {
    unsigned char c = (unsigned char)color & 0x0F;
    unsigned char c2 = (c | ((c << 4) & 0xF0));
    if ((x & 0x01) == 1)
    {
...
***************************************************************************************/
void TFT_eSprite::fillRect(int x, int y, int w, int h, unsigned int color)
{
  if (!_created || _vpOoB) return;
  ...
  if (_bpp == 16)
  {
    ...
    unsigned int iw = w;
    int ys = yp;
    if(h--)  {while (iw--) _img[yp++] = (unsigned short) color;}
    yp = ys;
    while (h--)
    {
      yp += _iwidth;
  ...
  else if (_bpp == 8)
  {
    ...
    while (h--)
    {
      memset(_img8 + yp, (unsigned char)color, w);
      yp += _iwidth;
    }
  }
  else if (_bpp == 4)
  {
    unsigned char c1 = (unsigned char)color & 0x0F;
    unsigned char c2 = c1 | ((c1 << 4) & 0xF0);
...
***************************************************************************************/
void TFT_eSprite::drawChar(int x, int y, unsigned short c, unsigned int color, unsigned int bg, unsigned char size)
{
...
//>>>>>>>>>>>>>>>>>>
  ...
  if ((size==1) && fillbg)
  {
    ...
    for (byte i = 0; i < 5; i++ ) column[i] = pgm_read_byte(font + (c * 5) + i);
    column[5] = 0;
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      //unsigned char  xa = pgm_read_byte(&glyph->xAdvance);
      short  xo16 = 0, yo16 = 0;
...
short TFT_eSprite::drawChar(unsigned short uniCode, int x, int y)
{
  return drawChar(uniCode, x, y, textfont);
}

  // Any UTF-8 decoding must be done before calling drawChar()
short TFT_eSprite::drawChar(unsigned short uniCode, int x, int y, unsigned char font)
{
...
***************************************************************************************/
//
void TFT_eSprite::drawGlyph(unsigned short code)
{
  unsigned short fg = textcolor;
  unsigned short bg = textbgcolor;
  bool getBG  = false;
  if (fg == bg) getBG = true;
  ...
  if (found)
  {
    ...
    const unsigned char* gPtr = (const unsigned char*) gFont.gArray;

#ifdef FONT_FS_AVAILABLE
    if (fs_font) {
      fontFile.seek(gBitmap[gNum], fs::SeekSet); // This is slow for a significant position shift!
      pbuffer =  (unsigned char*)malloc(gWidth[gNum]);
    }
#endif

    short cy = cursor_y + gFont.maxAscent - gdY[gNum];
    short cx = cursor_x + gdX[gNum];
...
** Description:             Write a string to the sprite cursor position
***************************************************************************************/
void TFT_eSprite::printToSprite(char *cbuffer, unsigned short len) //String string)
{
  if(!fontLoaded) return;

  unsigned short n = 0;
  bool newSprite = !_created;
  short  cursorX = _tft->cursor_x;
  ...
  if (newSprite)
  {
    ...
    while (n < len)
    {
      unsigned short unicode = decodeUTF8((unsigned char*)cbuffer, &n, len - n);
...
***************************************************************************************/
short TFT_eSprite::printToSprite(short x, short y, unsigned short index)
{
  bool newSprite = !_created;
  short sWidth = gWidth[index];
