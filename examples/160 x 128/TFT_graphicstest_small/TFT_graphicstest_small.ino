...
void testlines(unsigned short color) {
  tft.fillScreen(TFT_BLACK);
  for (short x=0; x < tft.width(); x+=6) {
    tft.drawLine(0, 0, x, tft.height()-1, color);
  }
  for (short y=0; y < tft.height(); y+=6) {
...
void testdrawtext(char *text, unsigned short color) {
  tft.setCursor(0, 0);
...
void testfastlines(unsigned short color1, unsigned short color2) {
  tft.fillScreen(TFT_BLACK);
  for (short y=0; y < tft.height(); y+=5) {
    tft.drawFastHLine(0, y, tft.width(), color1);
  }
  for (short x=0; x < tft.width(); x+=5) {
    tft.drawFastVLine(x, 0, tft.height(), color2);
  }
}

void testdrawrects(unsigned short color) {
  tft.fillScreen(TFT_BLACK);
  for (short x=0; x < tft.width(); x+=6) {
...
void testfillrects(unsigned short color1, unsigned short color2) {
  tft.fillScreen(TFT_BLACK);
  for (short x=tft.width()-1; x > 6; x-=6) {
    tft.fillRect(tft.width()/2 -x/2, tft.height()/2 -x/2 , x, x, color1);
    tft.drawRect(tft.width()/2 -x/2, tft.height()/2 -x/2 , x, x, color2);
  }
}

void testfillcircles(unsigned char radius, unsigned short color) {
  for (short x=radius; x < tft.width(); x+=radius*2) {
    for (short y=radius; y < tft.height(); y+=radius*2) {
      tft.fillCircle(x, y, radius, color);
    }
  }
}

void testdrawcircles(unsigned char radius, unsigned short color) {
  for (short x=0; x < tft.width()+radius; x+=radius*2) {
    for (short y=0; y < tft.height()+radius; y+=radius*2) {
